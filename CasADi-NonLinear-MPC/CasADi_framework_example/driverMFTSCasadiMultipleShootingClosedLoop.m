...
% Names and paths
name      = 'MFTSMultipleShootingClosedLoop';
figPath   = './figures/';
...
[opti, x, u, d, x0Cas] = casadiMultipleShooting(model, X0, U0, Ts, M, x0, ...
                                                        D(:,1:N), pCtrl);
% Output function
...
alpha = 0.995;
phiz  = sum( ( reshape( z           , nz*(N+1), 1 ) - ...
               reshape( zbar        , nz*(N+1), 1 ) ).^2 );
...
%% Closed-loop
% change horizo etc. -> chnges sim length
% Save for plotting
T = zeros(Ns*Nsim+1, 1);
T(1) = t0;
...
for k = 1:Ns
    ...
    % ---------------------- NMPC ---------------------- %
    % to do: put that into func "closed-loop func"
    % Measurement 
