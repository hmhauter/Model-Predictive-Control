...
zBars = ...
    [ 40.0, 20.0 ;
      20.0, 25.0 ];
zBar = zeros( nz, M*N+1 );
zBar(:,1:round((M*N)/2))     = repmat( zBars(:,1), 1, round((M*N)/2)   );
zBar(:,round((M*N)/2)+1:end) = repmat( zBars(:,2), 1, (M*N+1)-round((M*N)/2) );
...
[opti, x, u] = casadiCollocation(model, X0, U0, Ts, M, x0, D, theta);
...
phiz  = sum( ( reshape( z             , nz*(M*N+1), 1 ) - ...
               reshape( zBar          , nz*(M*N+1), 1 ) ).^2 )*Ts/M;
phidu = sum( ( reshape( u(:,2:end)    , nu*(N-1)  , 1 ) - ...
               reshape( u(:,1:end-1)  , nu*(N-1)  , 1 ) ).^2 )*Ts;
...
stairs( T/60, [ zBar(1,:) ], 'linewidth', ceil(lw/2), ...
    'displayname', '$\bar{z}_{1}$', ...
    'color', 'k', 'linestyle', ':'  )
stairs( T/60, [ zBar(2,:) ], 'linewidth', ceil(lw/2), ...
